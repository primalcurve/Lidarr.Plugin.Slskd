...
namespace NzbDrone.Core.Download.Clients.Slskd
{
    ...
    {
        SlskdConfig GetSettings(SlskdSettings settings);
        List<DownloadClientItem> GetQueue(SlskdSettings settings);
        string Download(string url, int bitrate, SlskdSettings settings);
    ...
    public class SlskdProxy : ISlskdProxy
    {
            ...
            Logger logger)
        {
            ...
            _userCache = cacheManager.GetCache<SlskdUser>(GetType(), "user");
            _httpClient = httpClient;
            _logger = logger;
        }

        public SlskdConfig GetSettings(SlskdSettings settings)
        {
            var request = BuildRequest(settings).Resource("/api/getSettings");
            var response = ProcessRequest<SlskdConfigResult>(request);
        ...
        public List<DownloadClientItem> GetQueue(SlskdSettings settings)
        {
            ...
            var response = ProcessRequest<SlskdQueue>(request);
        ...
        public string Download(string url, int bitrate, SlskdSettings settings)
        {
            ...
            var response = ProcessRequest<SlskdResult<SlskdAddResult>>(request);

            if (response.Result)
            {
                if (response.Data.Obj.Count != 1)
                {
                    throw new DownloadClientException("Expected Slskd to add 1 item, got {0}", response.Data.Obj.Count);
                }
            ...
            throw new DownloadClientException("Error adding item to Slskd: {0}", response.Errid);
        }

        private DownloadClientItem ToDownloadClientItem(SlskdQueueItem x)
        {
        ...
        private static DownloadItemStatus GetItemStatus(SlskdQueueItem item)
        {
            if (item.Failed > 0)
            {
        ...
        private TimeSpan? GetRemainingTime(SlskdQueueItem x, long size)
        {
            if (x.Progress == 100)
            {
        ...
        private HttpRequestBuilder BuildRequest(SlskdSettings settings)
        {
        ...
        private void Authenticate(string baseUrl, string arl)
        {
            ...
            if (user?.CurrentUser?.Name != null)
            {
                ...
                _logger.Debug("Already logged in to Slskd.");
                return;
            }
            ...
            if (cookies.ContainsKey("connect.sid"))
            {
                ...
                {
                    _userCache.Set(baseUrl, user.CurrentUser);
                    _logger.Debug("Slskd authentication succeeded");
                    return;
                }
            ...
            throw new DownloadClientException("Failed to authenticate with Slskd");
        }

        private SlskdConnect Connect(string baseUrl)
        {
            var requestBuilder = BuildRequest(baseUrl);
            requestBuilder.Resource("api/connect");

            var response = ProcessRequest<SlskdConnect>(requestBuilder);
